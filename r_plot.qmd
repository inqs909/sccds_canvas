...
To create a histogram, use the `hist()` funtiion. The `hist()` function only needs `x` argument which is numerical vector. Create a histogram with the `mpg` variable.
